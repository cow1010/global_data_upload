...
sheet_list = None
TARGET_SCHEMA = 'excel'
TARGET_TABLE = 'temp_sh_barcode_sale_global'

COLUMN_LIST_RAW = [
    'DT', 'On/Off', 'Posting Date', 'Plant', 'Barcode', 'Quantity', 'Sales Value', 'VND Tag Price'
]
COLUMN_LIST = [
    'distrbt_cd', 'on_off_cls', 'sale_dt', 'shop_id_global', 'barcode', 'sale_qty', 'sale_amt', 'tag_price'
]
COLUMN_LIST_FINAL = [
    'distrbt_cd', 'on_off_cls', 'sale_dt', 'shop_id_global', 'barcode', 'sale_qty', 'sale_amt', 'tag_price'
]
...
def read_excel2df(open_file_name):
    ...
    df['shop_id_global'] = df['shop_id_global'].replace(4529, 4503) #4529 매장명 -> 4503으로 삽입 요청 주심
    print(df)
    return df
